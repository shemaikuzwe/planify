...
export default function KanbanBoard({ taskId }: { taskId: string }) {
  const statuses = useLiveQuery(async () => {
    const statusArray = await db.taskStatus
      .where("categoryId")
      .equals(taskId)
      .toArray();
    ...
    const statusesWithTasks = statusArray
      .sort((a, b) => {
        if (
          (a as any).statusIndex !== undefined &&
          (b as any).statusIndex !== undefined
        ) {
          return (a as any).statusIndex - (b as any).statusIndex;
        }
        return (
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (type === "status") {
      ...
      itemsToUpdate.push({
        ...reorderedItem,
        statusIndex: destination.index,
      });
      if (source.index < destination.index) {
        itemsToUpdate.push({
          ...newStatuses[source.index],
          statusIndex: source.index,
        });
      } else {
        itemsToUpdate.push({
          ...newStatuses[destination.index + 1],
          statusIndex: destination.index + 1,
        });
      }
      await taskStore.updateStatusIndex(itemsToUpdate);
      return;
    }
    ...
    const sourceListIndex = newStatus.findIndex(
    ...
    try {
      if (sourceListId === destListId) {
        ...
        setStatus(newStatus);
        //Update in db
        await taskStore.updateTaskIndex(
          [
          ...
          { taskId: draggableId, statusId: destListId },
        );
      }
    } catch (error) {
