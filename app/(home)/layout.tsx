...
import { getRecentChats, getPinnedChats } from "@/lib/data/chat";


export default function layout({ children }: { children: React.ReactNode }) {
    const taskPromise = getUserSubtasks();
    const chatPromise = getRecentChats();
    const pinnedChatPromise = getPinnedChats();
    
    return (
        <div className="flex h-screen gap-2 w-full">
            <Navbar taskPromise={taskPromise} chatPromise={chatPromise} pinnedChatPromise={pinnedChatPromise} />
